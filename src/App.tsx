...
export default function App() {
  ...
  async function init() {
    try {
      ...
      if (configResult.data) {
        ...
        console.warn('Configuration OAuth InsForge indisponible:', configResult.error.message);
      }

      if (authResult.error) {
        setError(authResult.error.message || 'Session InsForge invalide.');
      } else if (authResult.data?.user) {
        setUser(authResult.data.user);
        await load(authResult.data.user.id);
      }
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
          : 'Initialisation InsForge impossible.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function oauth(provider: 'github' | 'google') {
    if (!oauthProviders.includes(provider)) {
      setError(`Le provider OAuth ${provider} n'est pas activé dans InsForge.`);
      return;
    }
  ...
  async function logout() {
    const { error: authError } = await api.auth.signOut();
    if (authError) {
      setError(authError.message || 'Déconnexion impossible.');
      return;
    }
    ...
    try {
      for (const file of Array.from(fileList)) {
        ...
        if (textual) {
          ...
          if (parts.length) {
            const { error: chunksError } = await api.database
            ...
            if (chunksError) {
              await api.database
                ...
                .update({
                  status: 'failed',
                  progress: 70,
                  current_stage: 'Échec indexation',
              ...
              throw new Error(
                chunksError.message || 'Impossible d’indexer le document.',
              );
            }
          }

          const { error: readyError } = await api.database
            .from('documents')
            .update({
              status: 'ready',
              progress: 100,
          ...
          if (readyError) {
            throw new Error(
              readyError.message || 'Impossible de finaliser le document.',
            );
          }
        } else {
          const jobId = crypto.randomUUID();
          const { error: jobError } = await api.database
            ...
            .insert({
              ...
              job_type: 'process',
              status: 'queued',
              stage: 'En attente du processeur PDF/DOCX',
              progress: 0,
              payload: {
          ...
          if (jobError) {
            await api.database
              ...
              .update({
                status: 'failed',
                current_stage: 'Échec mise en file',
            ...
            throw new Error(
              jobError.message || 'Impossible de créer le job de traitement.',
            );
          }

          queuedForWorker += 1;

          const { error: processError } = await api.functions.invoke(
            'process-document',
            {
              body: { jobId },
            },
          ...
          if (processError) {
            workerInvokeFailures += 1;
            console.warn(
              `Le job ${jobId} reste queued : Function process-document indisponible.`,
              processError.message,
            );
          }
        }

        const { error: auditError } = await api.database.from('audit_logs').insert({
          actor_id: user.id,
        ...
        if (auditError) {
          console.warn('Audit document.uploaded non enregistré:', auditError.message);
        }
      }

      if (workerInvokeFailures > 0) {
        setNotice(
          `Téléversement terminé. ${workerInvokeFailures}/${queuedForWorker} PDF/DOCX restent en file : la Function process-document n’a pas pu être invoquée.`,
        );
      } else if (queuedForWorker > 0) {
        setNotice(
          'Téléversement et traitement PDF/DOCX terminés. Actualisation des statuts…',
        );
      } else {
  ...
  async function remove(document: KnowledgeDocument) {
    ...
    const { error: markError } = await api.database
      .from('documents')
      .update({
        status: 'deleting',
        current_stage: 'Suppression du fichier',
      })
      .eq('id', document.id)
      .eq('owner_id', user.id);

    if (markError) {
      setError(markError.message || 'Suppression impossible.');
      return;
    }
    ...
    if (dbError) {
      setError(
        `${dbError.message || 'Suppression base impossible.'} Le fichier Storage a déjà été supprimé.`,
      );
      await load(user.id);
      return;
    }
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-5 py-7">
        ...
        <section className="rounded-3xl border bg-white p-5">
          ...
          <p className="mt-4 text-xs text-slate-500">
            Le lien seul ne révèle aucune clé. L’accès MCP distant est protégé
            par OAuth ; la clé statique éventuelle est réservée au debug serveur.
          </p>
        </section>
